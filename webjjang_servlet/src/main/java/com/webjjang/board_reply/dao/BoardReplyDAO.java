package com.webjjang.board_reply.dao;
...
//			pstmt = con.prepareStatement(LIST);
	...
	//2. 댓글 상세 보기는 리스트에서 내용이 다 표시 되어 필요하지 않는다./
	
	// 3. 글등록 처리
	// BoardReplyController - (Execute) - BoardReplyViewService - [BoardDAO.increase()]
	...
	public int update(BoardReplyVO vo) throws Exception{
		...
		try {
			...
			if(result == 0) { // 글번호가 존재하지 않는다. -> 예외로 처리한다.
				throw new Exception("예외 발생 : 댓글 번호 나 비밀번호가 맞지 않습니다. 정보를 확인해 주세요.");
			}
		} catch (Exception e) {
			e.printStackTrace();
			// 특별한 예외는 그냥 전달한다.
	...
	public int delete(BoardReplyVO vo) throws Exception{
		...
		try {
			...
			if(e.getMessage().indexOf("예외 발생") >= 0) throw e;
			// 그외 처리 중 나타나는 오류에 대해서 사용자가 볼수 있는 예외로 만들어 전달한다.
			else throw new Exception("예외 발생 : 게시판 댓글 삭제 DB 처리 중 예외가 발생했습니다.");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt);
		}
