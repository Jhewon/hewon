...
public class MessageDAO extends DAO{
	...
	public int setReaded (Long no) throws Exception{
		...
		try {
			...
			// 6. 표시 또는 담기
			System.out.println("MessageDAO.setReaded() - 메세지 카운트 1 감소 성공 ");
		} catch (Exception e) {
			e.printStackTrace();
			// 특별한 예외는 그냥 전달한다.
			if(e.getMessage().indexOf("예외 발생") >= 0) throw e;
			// 그외 처리 중 나타나는 오류에 대해서 사용자가 볼수 있는 예외로 만들어 전달한다.
			else throw new Exception("예외 발생 : 매세지 읽기 DB 처리 중 예외가 발생했습니다.");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt);
		}
	...
	// 2 - 3.   받은 날짜가 변경되면 회원의 새로운 메시지 개수 항목을 -1 해준다.
	// BoardController - (Execute) - BoardViewService - [BoardDAO.increase()]
	// 2-1의 결과 값이 1일때 실행
	public int descreaseNewMsgCnt (String id) throws Exception{
		// 결과를 저장할 수 있는 변수 선언.
		int result = 0;
		...
		try {
			...
			result = pstmt.executeUpdate();
			// 6. 표시 또는 담기
			System.out.println("MessageDAO.descreaseNewMsgCnt() - 메세지 카운트 1 감소 성공 ");
		} catch (Exception e) {
			e.printStackTrace();
			// 특별한 예외는 그냥 전달한다.
			if(e.getMessage().indexOf("예외 발생") >= 0) throw e;
			// 그외 처리 중 나타나는 오류에 대해서 사용자가 볼수 있는 예외로 만들어 전달한다.
			else throw new Exception("예외 발생 : 메시지 카운트 감소중 DB 처리 중 예외가 발생했습니다.");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt);
		}
	...
	public int write(MessageVO vo) throws Exception{
		...
		try {
			...
			// 6. 표시 또는 담기
			System.out.println();
			System.out.println("*** 메세지 보내기 완료 되었습니다. ***");
		} catch (Exception e) {
			e.printStackTrace();
			// 그외 처리 중 나타나는 오류에 대해서 사용자가 볼수 있는 예외로 만들어 전달한다.
			throw new Exception("예외 발생 : 메세지 보내기 DB 처리 중 예외가 발생했습니다.");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt);
		}
	...
	public int increaseNewMsgCnt(String id) throws Exception{
		...
		try {
			...
			result = pstmt.executeUpdate();
			// 6. 표시 또는 담기
			System.out.println("MessageDAO.increaseNewMsgCnt() - 메세지 카운트 1 증가 성공 ");
		} catch (Exception e) {
			e.printStackTrace();
			// 특별한 예외는 그냥 전달한다.
			if(e.getMessage().indexOf("예외 발생") >= 0) throw e;
	...
	final String LIST = "" 
		+ " select no, senderId, senderName, sendDate, accepterId , accepterName , acceptDate , senderPhoto , accepterPhoto  "
		+ " from ( "
			+ " select rownum rnum, no, senderId, senderName, sendDate, accepterId , accepterName , acceptDate , senderPhoto , accepterPhoto  "
			+ " from ( "
				+ " select m.no, m.senderId,sm.name senderName , "
				+ " sm.photo senderPhoto ,  "
				+ " to_char(m.sendDate, 'yyyy-mm-dd') sendDate, "
				+ " m.accepterId , am.name accepterName , "
				+ " am.photo accepterPhoto,  "
	...
	private String getListSQL(PageObject pageObject) {
		...
		// mode 조건 처리 . 1- 받은 메시지 , 2 - 보낸 매세지 , 3-모든 매세지
	...
	final String VIEW= ""
			+ " select m.no,m.content, m.senderId,sm.name senderName , "
			+ " sm.photo senderPhoto ,  "
			+ " to_char(m.sendDate, 'yyyy-mm-dd') sendDate, "
			+ " m.accepterId , am.name accepterName , "
			+ " am.photo accepterPhoto,  "
