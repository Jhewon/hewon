...
//		
		...
		// 파일시스템에서는 절대 저장위치가 필요하다. - 파일 업로드 시 필요.
		...
		try { // 정상 처리
			...
			case "/image/list.do":
				...
				// 이미지 게시판의 한페이지 이미지이 개수의 기본 값을 6으로 처리하자.
				// 중복처리 를 하는 것이다. - 앞에 데이터에 덮어쓰기가 된다.
			...
			case "/image/write.do":
				...
				// 일반데이터 수집(사용자->서버 : form - input - name) : multi 에서
				String title = multi.getParameter("title");
				String content = multi.getParameter("content");
				String fileName = multi.getFilesystemName("imageFile");
				// id 는 session 에서 받아야 한다. -> 위에서 처리
				...
				// 메세지 보내기
				session.setAttribute("msg", "이미지 등록이 완료 되었습니다.");
