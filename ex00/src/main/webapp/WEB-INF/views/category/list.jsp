...
$(function(){
	...
	$(".cate_edit").click(function(){
		...
		return false; // a tag 페이지 이동 이벤트를 무시 
	});
	
	// 대분류 추가 버튼
	$("#bigWriteBtn").click(function(e){
		//alert("대분류 추가");
		return categoryProcess("대분류 추가",0,0,"","write.do","추가");
	});
	// 대분류 추가 버튼
	$("#midWriteBtn").click(function(e){
	...
	// 중분류 수정 처리
		...
		//alert("cate_code1 = " + cate_code1 +  ", cate_name = " + cate_name + " , cate_code2 " + cate_code2);
...
<div class="container">
	<div class="card">
	  ...
	  <div class="card-body">
	  <p>
	  	대분류 탭을 수정하거나 삭제 하려면 오른쪽 마우스 클릭 하세요.
	  </p>
		<!-- Nav tabs -->
		  <ul class="nav nav-tabs">
			  <c:forEach items="${bigList }" var="vo">
			    <li class="nav-item">
			      ...
			      data-toggle="tab" data-cate_code1="${vo.cate_code1 }" href="#mid_category">
			      <span class="cate_name">${vo.cate_name }</span>
			      <i class="fa fa-edit cate_edit"></i>
				      <div class="editDiv">
					      <button class="btn btn-outline-dark btn-sm bigUpdateBtn">수정</button>
					      <br>
					      <button class="btn btn-outline-dark btn-sm bigDeleteBtn">삭제</button>
				      </div>
			      </a>
			    </li>
			  </c:forEach>
		  	<li class="nav-item">
		      <a class="nav-link " id="bigWriteBtn"
		      data-toggle="tab" data-cate_code1="${vo.cate_code1 }" href="#mid_category">
	  ...
	  <div class="tab-content">
	    <div id="mid_category" class="container tab-pane active"><br>
	      <h3>카테고리 - 중분류 
	      	<button class="btn btn-outline-dark btn-sm" id="midWriteBtn"
	      ...
	      <c:forEach items="${midList }" var="vo">
			  <li class="list-group-item" data-cate_code1="${vo.cate_code1 }" data-cate_code2="${vo.cate_code2 }">
			 	<sapn class="cate_name"> ${vo.cate_name }</sapn>
			 	 <span class="pull-right">
			 	 	<button class="btn btn-outline-dark btn-sm midUpdateBtn">수정</button>
			 	 	<button class="btn btn-outline-danger btn-sm midDeleteBtn">삭제</button>
			 	 </span>
			  </li>
	      </c:forEach>
		 </ul>
  ...
  <div class="modal-dialog">
	...
	<form method="post" id="modalForm">
	      ...
	      <div class="modal-footer">
	      	<button class="btn btn-dark" id="submitBtn">전송</button>
